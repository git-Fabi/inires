...
from src.utils.utils_repo_reader_agent import setup_repo_reader_agent, prepare_repo_reader_input
...
def runner(flock: Flock, ticket: Ticket) -> str:
    ...
    if isinstance(ticket_context_json, str):
        ticket_description = json.loads(ticket_context_json).get("description", ticket.title)
    else:
    ...
    repo_reader_input_str = prepare_repo_reader_input(ticket_description)
    repo_reader_output_json = flock.run(
        "repo_reader_agent",
        input={"repository+ticket_context": repo_reader_input_str}
    )
